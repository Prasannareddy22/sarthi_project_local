...
const SchemeDetail = () => {
  ...
  if (!scheme) return <div className="p-10 text-center">{t("schemeDetail.notFoundTitle")}</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 md:p-12 bg-white min-h-screen">
      ...
      >
        &larr; {t("schemeDetail.backToSchemesShort")}
      </Link>
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
          <h3 className="text-xl font-bold text-slate-900 mb-4">{t("schemeDetail.keyBenefits")}</h3>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
          ...
          <ul className="space-y-2">
            {scheme.documents.map((doc, i) => (
              <li key={i} className="text-slate-700 list-disc list-inside">
                {doc}
              </li>
            ))}
          </ul>
      ...
      <div className="mb-10 p-6 border-l-4 border-blue-600 bg-blue-50">
        <h3 className="text-xl font-bold text-blue-900 mb-2">{t("schemeDetail.howToApply")}</h3>
      ...
      {scheme.onlineLink && (
        ...
        >
          {t("schemeDetail.visitOfficialWebsite")}
        </a>
      )}
    </div>
  );
